...
describe('GradeCategoriesService', () => {
  ...
  it('rejects a blank category name', async () => {
    await expect(
      service.createGradeCategory({ classId: 1, name: '   ', weight: 20 }),
  ...
  it('rejects a duplicate category name in the same class (case-insensitive)', async () => {
    ...
    prisma.classGradeCategory.findFirst.mockResolvedValue({
      id: 1,
      name: 'homework',
    });

    await expect(
      service.createGradeCategory({ classId: 1, name: 'HOMEWORK', weight: 20 }),
    ).rejects.toBeInstanceOf(ConflictException);
  });

  it('rejects when the new category would push total weight over 100', async () => {
    prisma.class.findUnique.mockResolvedValue({ id: 1 });
    prisma.classGradeCategory.findFirst.mockResolvedValue(null);
    prisma.classGradeCategory.aggregate.mockResolvedValue({
      _sum: { weight: 90 },
    });

    await expect(
      service.createGradeCategory({ classId: 1, name: 'Exams', weight: 20 }),
    ).rejects.toBeInstanceOf(BadRequestException);
  ...
  it('creates the category when the running total weight stays at or under 100', async () => {
    ...
    expect(prisma.classGradeCategory.create).toHaveBeenCalledWith({
      data: { classId: 1, name: 'Exams', weight: 20 },
      include: { class: true },
    });
  });
});
