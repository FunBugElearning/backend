...
describe('UsersService', () => {
  ...
  let prisma: {
    ...
    role: { findUnique: jest.Mock; upsert: jest.Mock };
  };
  ...
  describe('createTeacher / createStudent', () => {
    it('createTeacher always resolves the teacher role, ignoring any client-supplied role_id', async () => {
      ...
      prisma.role.upsert.mockResolvedValue({ id: 2, name: 'teacher' });
      ...
      expect(prisma.role.upsert).toHaveBeenCalledWith(
        expect.objectContaining({ where: { name: 'teacher' } }),
      );
    ...
    it('createStudent always resolves the student role, ignoring any client-supplied role_id', async () => {
      prisma.user.findUnique.mockResolvedValue(null);
      prisma.role.upsert.mockResolvedValue({ id: 3, name: 'student' });
      prisma.user.create.mockResolvedValue({
        id: 1,
